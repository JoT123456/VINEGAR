...
function renderPieChart(containerEl, data, dict = {}) {
    ...
    drawPieLegend(containerEl, sortedData, colorMap, dict);
}
...
function drawPieLegend(containerEl, data, colorMap, dict) {
    const compoundWord = dict['chart.metabolite.compound'] || 'Compound';
    const othersWord = dict['chart.metabolite.others'] || 'Others';
    const compoundNames = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', othersWord];
    ...
    data.forEach((item, idx) => {
        ...
        label.textContent = `${compoundWord} ${compoundNames[idx]}`;
